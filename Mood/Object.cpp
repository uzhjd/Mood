...
private:
	...
	bool draw;
	float vertex[24][3] = {
		{0.0,1.5,-0.5},{-5.0, 1.5, -0.5},{-5.0, 1.5, -1.0},{0.0, 1.5, -1.0},
		{0.0,1.5,-0.5},{0.0, -1.5, -0.5},{0.0, -1.5, -1.0},{0.0, 1.5, -1.0},
		{0.0,-1.5,-0.5},{-5.0, -1.5, -0.5},{0.0, 1.5, -1.0},{0.0, -1.5, -1.0},
		{-5.0,1.5,-0.5},{-5.0,  -1.5, -0.5},{-5.0, -1.5, -1.0},{-5.0, 1.5, -1.0},
		{0.0,1.5,-0.5},{-5.0, 1.5, -0.5},{-5.0, -1.5, -0.5},{0.0, -1.5, -0.5},
		{0.0, 1.5,-1.0},{-5.0, 1.5, -1.0},{-5.0, -1.5, -1.0},{0.0, -1.5, -1.0}
	};
	float bvertex[24][3] = {
...
public:
	...
	void collision_pepero(float& p1x, float& p1y, float& p1z, float& p2x, float& p2y, float& p2z) {
		...
		if (draw) {
			if (p1x + 0.5 >= (x - 5.0) && p1x - 0.5 <= x) { //빼빼로 x값 사이에 p1이 있을 때
				if (z - 0.5 >= p1bottom) {
					p1z = z + 0.5;
				}
			...
			if (p2x + 0.5 >= (x - 5.0) && p2x - 0.5 <= x) { //빼빼로 x값 사이에 p2이 있을 때
